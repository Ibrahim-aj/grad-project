...
require_once('../db.php');
	...
	$countStudent = mysqli_query($conn, "SELECT * FROM student");

?>
<!DOCTYPE html>
<html>
<title>حذف الطلاب</title>
    <head>
        <meta charset="UTF-8">
...
<div class="icon-bar w3-teal">
 ...
 <h1 class="page-header"><font face="changa">حذف الطلاب</font></h1>
    ...
    <div class="w3-container" style="background-color:#D8DCF8">
                ...
                <div class="col-sm-9 col-sm-offset-3 col-md-8 col-md-offset-2 ">
                  <center><div class="btn btn-default " style="margin-bottom:15px;">عدد الطلاب المسجلين : <strong class="text-primary"><?php echo mysqli_num_rows($countStudent); ?></strong></div></center>
...
<?php
	...
	if(isset($_POST['from']) && $_POST['from'] == "DELETE"){
		if(isset($_POST['student_id'])){
			...
			// استعلام عن البحث اذا يوجد حجوزات للطالب
			$query_Student_booking = mysqli_query($conn, "SELECT * FROM booking WHERE student_id = '{$student_id}' AND canceled = 0 ");
			if(mysqli_num_rows($query_Student_booking) > 0){
				echo "انت لاتستطيع حذف الطالب لان يوجد لديه حجز"; 
			}else{
			
			
			
			$query_Student = mysqli_query($conn, "SELECT * FROM student WHERE student_id = '{$student_id}'");
			if(mysqli_num_rows($query_Student) != 0){
				$roww = mysqli_fetch_array($query_Student);
				$addApp = mysqli_query($conn, " DELETE FROM student WHERE student_id = '{$student_id}'");
				if($addApp){
					mysqli_query($conn, " DELETE FROM user WHERE user_id = '{$roww['user_id']}'");
					echo '<div class="alert alert-success">تم حذف الطالب بنجاح</div>';
				}
			}else{
				echo '<div class="alert alert-danger">رقم الطالب غير صحيح يرجى التأكد من الرقم</div>';
			}
		}
	}
	}
?>
			  ...
			  <div class="form-group text-center">
                   ...
                   <b> أدخل رقم الطالب الجامعي :   </b> <input type="text" name="student_id" id="student_id" >
                  
               
				  <br> </br>
              
                      <button type="submit" name="from" class="btn btn-primary btn-lg" style="font-size: 14px" value="DELETE">حذف الطالب</button>
                  </div>
                  </form>

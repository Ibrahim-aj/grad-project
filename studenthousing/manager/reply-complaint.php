...
require_once('../db.php');
	...
	$countComplaint = mysqli_query($conn, "SELECT * FROM complaint");
	if(isset($_GET['id']) && mysqli_real_escape_string($conn, $_GET['id'] == "")){
		header("location: show-complaint.php");
		exit;
	}
	$id = mysqli_real_escape_string($conn, $_GET['id']);
	$queryEdit = mysqli_query($conn, "SELECT * FROM complaint WHERE complaint_id = '{$id}'");
...
<html>
    ...
    <!-- GOOGLE FONT-->
        ...
        <title>الرد على الشكوى رقم<?=$row['complaint_id'];?></title>
    </head>
...
<div class="icon-bar w3-teal">
    ...
    <div class="w3-container" style="background-color:#D8DCF8">
                ...
                <div class="col-sm-9 col-sm-offset-3 col-md-8 col-md-offset-2 ">
                  <center><div class="btn btn-default " style="margin-bottom:15px;">عدد الشكاوي الحالية : <strong class="text-primary"><?php echo mysqli_num_rows($countComplaint); ?></strong></div></center>
                  <form method="post" action="reply-complaint.php?id=<?=$row['complaint_id'];?>">
<?php


	if(isset($_POST['from']) && $_POST['from'] == "reply"){
		if((isset($_POST['reply']) && $_POST['reply'] == "")){
			echo '<div class="alert alert-danger">يرجى كتابة الرد </div>';
		}else{
			$reply = mysqli_real_escape_string($conn, $_POST['reply']);
			$addApp = mysqli_query($conn, "UPDATE complaint SET reply = '{$reply}', status = '1' WHERE complaint_id = '{$id}'");
			if($addApp){
				echo '<div class="alert alert-success">تمت اضافة الرد على الشكوى بنجاح</div>';
			}
		}
	}
	$queryEdit = mysqli_query($conn, "SELECT * FROM complaint WHERE complaint_id = '{$id}'");
...
?>
			<div class="panel panel-primary">
        		<div class="panel-heading">رقم الطالب المشتكي : <b><?=$row['student_id'];?></b>
                	<span class="pull-left">الشكوى رقم : <b><?=$row['complaint_id'];?></b></span>
                </div>
                <div class="panel-body">
                	<p><b><?=$row['complaint_text'];?></b></p>
                    <hr />
                    <h3>رد الادارة</h3>
                    <p><?php if($row['reply']){ echo $row['reply']; }else{ echo "شكوى بدون رد"; } ?></p>
                </div>
                <div class="panel-footer">
                  <div class="form-group">
                    <h5 class="text-center">الرد على الشكوى</h5>
                    <textarea name="reply" class="form-control" rows="3"><?php if($row['reply']){ echo $row['reply']; } ?></textarea>
                  </div>
                  <div class="form-group text-center">
                      <button type="submit" name="from" class="btn btn-primary btn-lg" value="reply">اعتماد الرد</button>
                  </div>
                </div>
              </div>
                  </form>

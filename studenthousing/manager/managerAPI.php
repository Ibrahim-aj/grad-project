...
function sh_managers_get($f = '*',$extra = '')
{
    global $conn;
    $ex = strip_tags($extra);
    $query = sprintf("SELECT %s FROM `manager` %s ",$f,$extra); 
    $qresult = mysqli_query($conn, $query);
...
function sh_managers_get_by_id($uid)
{
    global $conn;
    $id = mysqli_real_escape_string($conn ,strip_tags($uid));
        
    $result = sh_users_get('*', 'WHERE `manager_id` = '."'$id'");
    if($result == NULL)
        return NULL;
...
function sh_manager_get_by_user_id($user_id)
{
    global $conn;
    $id = mysqli_real_escape_string($conn ,strip_tags($user_id));
    
    $query = sprintf("SELECT * FROM `manager` WHERE user_id = '%s'",$id); 
    $qresult = mysqli_query($conn, $query);
...
function sh_manager_get_by_email_from_users($email)
{
    global $conn;
...
function sh_managers_get_by_phone_number_from_users($email)
{
    global $conn;
    $n_email = mysqli_real_escape_string($conn ,strip_tags($email));
...
function sh_managers_add($fname, $minit, $lname, $email, $password, $dob, $phone, $student_id)
{
    global $conn;
    ...
    $query = sprintf("INSERT INTO `manager` VALUE('%s','%s',NULL)",$user_student_id->user_id, $n_student_id);
...
function sh_managers_delete($uid)
{
    global $conn;
    $id = mysqli_real_escape_string($conn ,strip_tags($uid));
        
    $query = sprintf("DELETE FROM `manager` WHERE `manager_id` = %d",$id);
...
function sh_managers_update($uid, $fname = NULL, $minit  = NULL, $lname  = NULL, $email  = NULL, $password  = NULL, $dob  = NULL, $phone  = NULL, $student_id  = NULL)
{
    global $conn;
    $id = mysqli_real_escape_string($conn ,strip_tags($uid));

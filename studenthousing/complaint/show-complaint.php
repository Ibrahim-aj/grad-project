...
require_once('../db.php');
	...
	$queryEdit = mysqli_query($conn, "SELECT * FROM complaint WHERE complaint_id = '{$id}' AND student_id = '{$_SESSION['LoginStudent']}'");
...
<div class="color_forside w3-sidebar w3-bar-block w3-card-2 w3-animate-right" style="display:none;right:0;" id="rightMenu">
    ...
    <a href="../complaint/Complaint.php" class="w3-bar-item w3-button  w3-border">عرض الشكوى</a>
...
<div class="icon-bar w3-teal">
    ...
    <div class="w3-container" style="background-color:#D8DCF8">
                ...
                <div class="col-sm-9 col-sm-offset-3 col-md-8 col-md-offset-2 ">
                    <h1 class="page-header"><font face="changa">عرض الشكوى</font></h1>

					<?php
	$queryCmp = mysqli_query($conn, "SELECT * FROM complaint WHERE complaint_id = '{$id}' AND student_id = '{$_SESSION['LoginStudent']}'");
	$row = mysqli_fetch_assoc($queryCmp);
?>
			<div class="panel panel-primary">
        		<div class="panel-heading">المشتكي : <b><?=$row['student_id'];?></b>
                ...
                <div class="panel-body">
                    ...
                    <hr />
                    <h3>رد الادارة</h3>
                    <p><?php if($row['reply']){ echo $row['reply']; } ?></p>
                    <hr />
                    <?php if($row['status'] == 1){ ?>
                    <p class="label label-success">تم الرد</p>
                     <?php }else{ ?>
                    <p class="label label-warning">في انتظار الرد</p>
                     <?php } ?>
                </div>
              </div>

...
<div class="icon-bar w3-teal">
 ...
 <?php
	if(isset($_POST['from']) && $_POST['from'] == "SendReport" ){
		$textarea1 = strip_tags($_POST['textarea1']);
		$username = mysqli_real_escape_string($conn, $textarea1);
		if(isset($textarea1) && $textarea1 != ''){
			$insert = mysqli_query($conn, "INSERT INTO complaint( complaint_text , student_id  )
VALUES ( '$textarea1' , '{$_SESSION['LoginStudent']}' )");
			if($insert){
				$msgError = '<div class="alert alert-success">تم ارسال الشكوى بنجاح</div>';
			}else{
